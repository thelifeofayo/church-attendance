...
class DashboardService {
    ...
    async getHODDashboard(currentUser) {
        ...
        if (!department) {
            throw new errors_1.ForbiddenError('Department not found');
        }
        // Get current week attendance records
        const [wednesdayRecord, sundayRecord] = await Promise.all([
            prisma_1.prisma.attendanceRecord.findFirst({
                where: {
                    departmentId: currentUser.departmentId,
                    serviceDate: week.wednesday,
                    serviceType: shared_1.ServiceType.WEDNESDAY,
                },
                include: {
            ...
            prisma_1.prisma.attendanceRecord.findFirst({
                where: {
                    ...
                    serviceType: shared_1.ServiceType.SUNDAY,
                },
                include: {
                    entries: { include: { member: { select: { firstName: true, lastName: true } } } },
                    _count: { select: { entries: true } },
        ...
        const recentSubmissions = await prisma_1.prisma.attendanceRecord.findMany({
            where: {
                departmentId: currentUser.departmentId,
    ...
    async getTeamHeadDashboard(currentUser) {
        ...
        if (!team) {
            throw new errors_1.ForbiddenError('Team not found');
        }
        ...
        // Calculate summaries
        const wednesdayRecords = records.filter((r) => r.serviceType === shared_1.ServiceType.WEDNESDAY);
        const sundayRecords = records.filter((r) => r.serviceType === shared_1.ServiceType.SUNDAY);
        const calculateSummary = (recs, serviceDate) => {
            ...
            const pending = recs.filter((r) => r.status === shared_1.SubmissionStatus.NOT_STARTED).length;
            const notSubmitted = recs.filter((r) => r.status === shared_1.SubmissionStatus.NOT_SUBMITTED).length;
            return {
        ...
        const departmentBreakdown = team.departments.map((dept) => {
            ...
            return {
                department: {
                    ...
                    teamId: dept.teamId,
                    hodId: dept.hodId,
                    assistantHodId: dept.assistantHodId,
    ...
    async getAdminDashboard(_currentUser) {
        const week = this.getWeekBounds();
        // Get totals
        const [totalTeams, totalDepartments, totalMembers] = await Promise.all([
            prisma_1.prisma.team.count({ where: { isActive: true } }),
            prisma_1.prisma.department.count({ where: { isActive: true } }),
            prisma_1.prisma.member.count({ where: { isActive: true } }),
        ]);
        ...
        const calculateOverallSummary = (recs, serviceDate) => {
            ...
            return {
                ...
                wednesday: {
                    ...
                    submissionCompleteness: deptCount > 0
                        ? Math.round((teamWedRecs.filter((r) => r.status === shared_1.SubmissionStatus.SUBMITTED).length / deptCount) * 100)
                        : 0,
                },
                sunday: {
                    ...
                    percentage: teamMemberCount > 0
                        ...
                        : 0,
                    submissionCompleteness: deptCount > 0
                        ? Math.round((teamSunRecs.filter((r) => r.status === shared_1.SubmissionStatus.SUBMITTED).length / deptCount) * 100)
                        : 0,
                },
            };
        });
