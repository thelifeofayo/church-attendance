...
interface AttendanceDetail {
  ...
  absent: number;
  percentage: number;
  absentMembers?: { name: string; reason: string | null }[];
}

export class ReportsService {
  private getWeekBounds(weekStart: string): { start: Date; end: Date; wednesday: Date; sunday: Date } {
  ...
  async getWeeklyReport(query: WeeklyReportQuery): Promise<WeeklyReportData> {
    ...
    // Build report data
    const reportTeams: TeamReportData[] = teams.map((team) => {
      const teamRecords = records.filter((r) =>
        team.departments.some((d) => d.id === r.departmentId)
      );

      const departments: DepartmentReportData[] = team.departments.map((dept) => {
        const wedRec = teamRecords.find(
          (r) => r.departmentId === dept.id && r.serviceType === ServiceType.WEDNESDAY
        );
        const sunRec = teamRecords.find(
          (r) => r.departmentId === dept.id && r.serviceType === ServiceType.SUNDAY
        );

        const buildDetail = (rec: typeof wedRec): AttendanceDetail | null => {
          if (!rec) return null;
        ...
        return {
          ...
          wednesday: serviceType === ServiceType.SUNDAY ? null : buildDetail(wedRec),
          sunday: serviceType === ServiceType.WEDNESDAY ? null : buildDetail(sunRec),
        };
      });

      // Team totals
      const teamMemberCount = team.departments.reduce((sum, d) => sum + d._count.members, 0);
      const wedRecords = teamRecords.filter((r) => r.serviceType === ServiceType.WEDNESDAY);
      const sunRecords = teamRecords.filter((r) => r.serviceType === ServiceType.SUNDAY);
    ...
    const totalMembers = teams.reduce(
      (sum, t) => sum + t.departments.reduce((s, d) => s + d._count.members, 0),
      0
    );
    const totalDepts = teams.reduce((sum, t) => sum + t.departments.length, 0);

    const wedRecords = records.filter((r) => r.serviceType === ServiceType.WEDNESDAY);
    const sunRecords = records.filter((r) => r.serviceType === ServiceType.SUNDAY);

    const buildSummary = (recs: typeof records, date: Date): ServiceSummary | null => {
      if (recs.length === 0 && serviceType && serviceType !== (date === week.wednesday ? ServiceType.WEDNESDAY : ServiceType.SUNDAY)) {
        return null;
      }
    ...
    return {
      ...
      summary: {
        wednesday: serviceType === ServiceType.SUNDAY ? null : buildSummary(wedRecords, week.wednesday),
        sunday: serviceType === ServiceType.WEDNESDAY ? null : buildSummary(sunRecords, week.sunday),
      },
      teams: reportTeams,
    };
  }

  async generatePDF(query: ExportReportQuery): Promise<Buffer> {
  ...
  async generateCSV(query: ExportReportQuery): Promise<string> {
    const report = await this.getWeeklyReport(query);
        ...
        if (report.summary.wednesday) {
          if (dept.wednesday) {
            const absentList = dept.wednesday.absentMembers
              ...
              .join('; ') || '';
            row.push(
              `"${report.summary.wednesday.serviceDate}"`,
              dept.wednesday.status,
              dept.wednesday.present.toString(),
              dept.wednesday.absent.toString(),
              `${dept.wednesday.percentage}%`,
              `"${absentList}"`
            );
          } else {
            row.push('', '', '', '', '', '');
        ...
        if (report.summary.sunday) {
          if (dept.sunday) {
            const absentList = dept.sunday.absentMembers
              ...
              .join('; ') || '';
            row.push(
              `"${report.summary.sunday.serviceDate}"`,
              dept.sunday.status,
              dept.sunday.present.toString(),
              dept.sunday.absent.toString(),
              `${dept.sunday.percentage}%`,
              `"${absentList}"`
            );
          } else {
            row.push('', '', '', '', '', '');
